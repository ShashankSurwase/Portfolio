...
const PROJECTS: Project[] = [
  ...
  { id: "ecom_multi_marketplace_airflow_etl", title: "Multi-Marketplace Airflow ETL Platform (Flagship)", domain: "E-commerce", impact: "Airflow 131-DAG production platform ingesting 31 source systems into PostgreSQL & ClickHouse dual warehouse.", tags: ["Apache Airflow", "PostgreSQL", "ClickHouse", "AWS S3"], repo: repoUrl("E-commerce", 1) },
  { id: "ecom_unified_sales_inventory_facts", title: "Multi-Platform Sales, DIVAS Inventory & BI Analytics Suite", domain: "E-commerce", impact: "Unified 12 sales platforms into a 129-col all_sales fact and 9 facility types into a 92-col DIVAS inventory engine.", tags: ["ClickHouse", "DIVAS Engine", "Apache Superset", "Power BI"], repo: repoUrl("E-commerce", 2) },
  { id: "ecom_revenue_integration_recovery", title: "Multi-Platform Revenue Integration & $1.05M Recovery", domain: "E-commerce", impact: "Unified $3.78M across 6 platforms; fixed ASIN character-encoding bug to recover $1.05M unattributed revenue.", tags: ["Python", "pandas", "ASIN Recovery", "Data Quality"], repo: repoUrl("E-commerce", 6) },
  { id: "ecom_columnar_data_warehouse", title: "Columnar E-commerce Data Warehouse (30+ Tables)", domain: "E-commerce", impact: "From-scratch ClickHouse warehouse with 3 append-safe write patterns powering 27 production reports.", tags: ["ClickHouse", "Data Warehousing", "Python", "SQL"], repo: repoUrl("E-commerce", 3) },
  { id: "ecom_customer_intelligence_analytics", title: "Customer Intelligence & RFM Segmentation Engine", domain: "E-commerce", impact: "Scikit-learn RFM segmentation, market-basket product adjacency and time-series demand forecasting.", tags: ["scikit-learn", "RFM", "R", "Demand Forecast"], repo: repoUrl("E-commerce", 4) },
  { id: "ecom_inventory_analytics_powerbi", title: "Inventory Intelligence & Version-Controlled Power BI", domain: "E-commerce", impact: "Ageing, ABC classification, True ADS velocity and automated reorder alerts before stockout.", tags: ["Power BI", "ClickHouse", "Inventory BI"], repo: repoUrl("E-commerce", 5) },

  // 🎓 EDTECH & ACADEMIC ANALYTICS DOMAIN
  { id: "edtech_multi_source_etl_platform", title: "Multi-Source EdTech Student Data ETL Platform", domain: "EdTech", impact: "Dual PostgreSQL (47 views) & ClickHouse (16 tables) platform wiring 7 source systems across ~2,200 students.", tags: ["Apache Airflow", "Playwright", "PostgreSQL", "ClickHouse"], repo: repoUrl("EdTech", 1) },
  { id: "edtech_student_analytics_dashboards", title: "Unified Student Analytics, BI & Automated Reporting Suite", domain: "EdTech", impact: "17 Superset tabs, 5-type student persona engine, OMR scoring engine, and closed-loop PDF scorecard delivery.", tags: ["Grafana", "Apache Superset", "OpenCV OMR", "ReportLab"], repo: repoUrl("EdTech", 2) },
  { id: "edtech_automated_omr_evaluation", title: "Automated Computer Vision OMR Evaluation System", domain: "EdTech", impact: "OpenCV & Tesseract OCR pipeline auto-scoring bubble-sheet exams with >97% accuracy and same-day results.", tags: ["OpenCV", "Tesseract OCR", "Python", "SQL"], repo: repoUrl("EdTech", 1) },
  { id: "edtech_swot_question_paper_generator", title: "SWOT-Based Personalised Question Paper Generator", domain: "EdTech", impact: "Derives per-student SWOT weakness profile and generates 500+ unique practice PDFs via ReportLab.", tags: ["Python", "ReportLab", "AWS S3", "Fuzzy Matching"], repo: repoUrl("EdTech", 5) },

  // ☀️ RENEWABLE ENERGY (SOLAR) DOMAIN
  { id: "energy_iot_ingestion_pipeline", title: "Solar IoT Data Ingestion Pipeline (245 Plants)", domain: "Energy", impact: "Serverless IoT telemetry ingestion into Redshift Data API; cut Lambda invocations 864k → 8.6k/mo (−99%).", tags: ["AWS Lambda", "Amazon Redshift", "EventBridge", "Modbus"], repo: repoUrl("Energy", 1) },
  { id: "energy_solar_kpi_engine_pr_audit", title: "IEC 61724 Solar KPI Engine & PR Audit", domain: "Energy", impact: "Audited 13 dashboards to correct 6–17x Performance Ratio calculation bugs to within ±2% of IEC standard.", tags: ["IEC 61724 Standard", "Redshift", "Python", "SQL"], repo: repoUrl("Energy", 4) },
  { id: "energy_realtime_streaming_migration", title: "Real-Time Streaming Ingestion Migration (Kinesis)", domain: "Energy", impact: "Producer → Kinesis → Redshift streaming MV cutting latency to seconds with exactly-once shard watermarks.", tags: ["AWS Kinesis", "Redshift Streaming", "Exactly-Once"], repo: repoUrl("Energy", 10) },
  { id: "energy_operations_automation", title: "Solar Operations Automation Suite (DGR & Invoicing)", domain: "Energy", impact: "Daily generation reports, tariff-based invoicing, and real-time absence alarm engine (95+ hrs/mo saved).", tags: ["AWS Lambda", "Redshift", "SES", "FastAPI"], repo: repoUrl("Energy", 2) },
  { id: "energy_role_based_dashboard_suite", title: "Role-Based EMS Dashboard Suite (13 Surfaces)", domain: "Energy", impact: "13 Grafana dashboard surfaces for 12 user roles with query-level RBAC enforced in FastAPI SQL backend.", tags: ["Grafana", "FastAPI", "RBAC", "Redshift"], repo: repoUrl("Energy", 3) },
  { id: "energy_platform_overview", title: "Utility-Scale Solar EMS — Platform Overview", domain: "Energy", impact: "End-to-end cloud EMS for 245 solar plants: IoT ingestion, IEC 61724 KPI engine, RBAC & ops automation.", tags: ["AWS", "Redshift", "Grafana", "Full Stack"], repo: repoUrl("Energy", 5) },
];
...
const DETAILS: Record<string, { blurb: string; results: string[] }> = {
  ...
  energy_realtime_streaming_migration: { blurb: "A real-time streaming ingestion lane running alongside the batch pipeline, cutting plant-to-warehouse latency from minutes to seconds. Per-family producer Lambdas publish a compact grouped-JSON payload — 86% smaller than the CSV and 321× fewer records — to a Kinesis stream partitioned on site name, which Redshift consumes natively through an external schema and an auto-refreshing streaming materialized view. A per-shard sequence-number watermark makes every load exactly-once, so replays and refreshes never duplicate a row; the 740M-row landing table is tuned for near-zero vacuum.", results: ["Latency minutes → seconds", "Payload 86% smaller · 321× fewer records", "Exactly-once via shard watermark", "740M-row landing, near-zero vacuum"] },
