...
const PROJECTS: Project[] = [
  ...
  },
  {
    ...
    title: "Unified Sales, INVMS Inventory & ClickHouse Analytics Platform",
    domain: "E-commerce",
    impact: "ClickHouse Single Source of Truth (SSOT) data platform harmonizing 12 revenue platforms into all_platform_sales (129-col) via date-level UNIONs, partner-to-child SKU mapping (ASIN/FSN/Style ID), and INVMS inventory (92-col) across 9 facility types and 4 Reliance sub-channels (Ajio, Azorte, Tira Off/On). Engineered 3 explicit ClickHouse write patterns (TRUNCATE+INSERT, append+OPTIMIZE, deleteInsert) with an hourly Airflow DAG (15-day rolling DELETE+INSERT mutation window), powering predictive ML feature datasets and 28 team KPI dashboards with $1.05M recovered in unattributed UAE revenue.",
    tags: ["ClickHouse SSOT (129-col)", "all_platform_sales Fact", "INVMS Inventory (92-col)", "3 Write Patterns", "15-Day DELETE+INSERT Window", "Predictive ML Layer", "$1.05M ASIN Recovery", "28 Team Dashboards"],
...
const DETAILS: Record<string, { blurb: string; results: string[] }> = {
  ...
  ecom_unified_sales_inventory_facts: {
    blurb: "Designed and engineered a ClickHouse OLAP data warehouse from scratch acting as the Single Source of Truth (SSOT) for all team analytics, predictive ML forecasting, and executive dashboarding across 12+ sales channels and 9 inventory facility types. The all_platform_sales pipeline harmonizes raw tables across 12 platforms via date-level UNIONs and maps partner SKUs (ASINs, FNSKUs, Style IDs) to internal master Child SKUs with revenue disaggregation down to individual child items. The INVMS inventory fact consolidates 9 facility types with True-ADS calculation and 36 platform-ADS columns powering PO replenishment. Engineered 3 explicit write patterns suited for ClickHouse OLAP semantics—TRUNCATE+INSERT for snapshots (INVMS), putDataframe()+OPTIMIZE TABLE for appends, and deleteInsert() for date-keyed operational tables. An hourly Airflow DAG ingests live 1-hour sales while executing a 15-day rolling DELETE+INSERT mutation window, backed by parameterised on-demand historical backfill DAGs. Consolidated 4 Reliance sub-channels (Ajio, Azorte, Tira Off/On) for the first time into a single view and recovered $1.05M in unattributed UAE marketplace revenue by fixing an ASIN character-encoding bug (match rate 0% → 99.8%). Powers 28 production BI dashboards across sales, inventory, marketing ROAS, and return diagnostics.",
    results: ["ClickHouse SSOT (129/92 cols)", "3 ClickHouse Write Patterns", "all_platform_sales & INVMS", "15-Day DELETE+INSERT Window", "$1.05M Revenue Recovered", "28 Production BI Dashboards"],
  },
